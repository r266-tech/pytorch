...
def filter_shapes(shapes: list) -> list:
    # Determine minimum size requirement based on device
    shared_memory = get_gpu_shared_memory()
    device_capa = torch.cuda.get_device_capability()

    if shared_memory == 166912 and device_capa == (8, 0):
        min_size = 512  # A100
        device_name = "A100"
    elif shared_memory == 232448 and device_capa == (9, 0):
        min_size = 768  # H100
        device_name = "H100"
    else:
        min_size = 0    # Unknown device, no size requirement
        device_name = "Unknown"

    print(f"Device: {device_name}, minimum size requirement: {min_size}")
    ...
    for m, k, n, dtype in shapes:
        ...
        # Check if too small for precondition
        if min_size > 0 and (m < min_size or k < min_size or n < min_size):
            too_small_count += 1
            continue
    ...
    print(f"  Too small for precondition (skipped): {too_small_count}")
