...
def _check_equal(
    ...
    if golden.is_cuda and golden.dtype == torch.float32 and torch.backends.cuda.math_sdp.fp32_precision != "ieee":
        raise ValueError(
            "Testing script error: FP32 golden tensor must be calculated with IEEE precision. "
            "Add @tf32(math_sdp_precision='ieee') to related tests to fix it."
        )
